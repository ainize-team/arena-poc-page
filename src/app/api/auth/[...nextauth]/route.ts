...
const getJWTFromServer = async (accessToken: string) => {
  ...
  try {
    ...
    access_token.expire = Math.round(
      access_token.expire * JS_PYTHON_TIMESTAMP_GAP,
    ); // FIXME(yoojin): change to JS_PYTHON_TIMESTAMP_GAP in constants
    refresh_token.expire = Math.round(
      refresh_token.expire * JS_PYTHON_TIMESTAMP_GAP,
    );
    return {
  ...
  callbacks: {
    async jwt({ token, account }) {
      const googleAccessToken =
        account?.access_token || token.googleAccessToken;
      if (googleAccessToken) {
        ...
        if (
          !token.refreshToken ||
          token.refreshToken.expire < curMs - 60 * ONE_SEC_AS_TIMESTAMP
        ) {
          const { refreshToken, accessToken } =
            await getJWTFromServer(googleAccessToken);
          token = Object.assign({}, token, {
        ...
        if (
          !token.accessToken ||
          token.accessToken.expire < curMs - 60 * ONE_SEC_AS_TIMESTAMP
        ) {
